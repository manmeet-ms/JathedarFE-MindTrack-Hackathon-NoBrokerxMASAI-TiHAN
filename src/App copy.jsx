...
const App = () => {
 ...
 const { currentRank, nextRank } = getRankUtil(points);

 useEffect(() => {
 if (points && nextRank?.minPoints) {
  const percent = Math.floor((points / nextRank.minPoints) * 100);
 ...
 <span className="absolte">dont be an NPC, take back control </span>
   ...
   <section className=" flex flex-col gap-2 mt-2">
    <div className="bg-card flex gap-4 border rounded-lg p-2">
    <IconMilitaryAward className="bg-accent/60 p-2 size-10 rounded-full text-primary" />
    {/* <div className="bg-accent justify-center items-center rounded-full p-2 ">
    </div> */}
    <div className="flex flex-col">
     ...
     <Tooltip>
      ...
      {currentRank?.name} <IconInfoCircle size={16} strokeWidth={1.5} className="bg-accent/60 inline rounded-full ml-2 text-primary" />{" "}
      </TooltipTrigger>
      <TooltipContent>
      <span className="text-xs">
       {Math.abs(currentRank?.minPoints - points)} points to become <br /> {nextRank.name}
      </span>
      </TooltipContent>
     </Tooltip>
     </TooltipProvider>
     ...
     <span className="text-[11px]">{progressValue}%</span>
     </div>
    </div>
    </div>
   ...
   <div className="changed-px pb-4 flex items-center justify-between gap-2 px-4">
    ...
    <h4 className="scroll-m-20 text-xl font-semibold tracking-tight">{meta.title || Greet}</h4>
    <p className="leading-7 text-secondary-foreground text-sm opacity-40">{meta.subtitle || dayjs().format("MMM DD, YYYY hh:mm A")}</p>{" "}
    </div>{" "}
